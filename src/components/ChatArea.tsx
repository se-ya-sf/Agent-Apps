...
import { CLUB_TOOLS } from '@/lib/clubTools';
import { 
  Send, 
  Loader2, 
  User, 
...
const TOOL_ICONS: Record<string, React.ReactNode> = {
  ...
  club_save_preference: <Wine className="w-4 h-4" />,
  club_get_preference: <BookOpen className="w-4 h-4" />,
  club_save_experience: <Beer className="w-4 h-4" />,
  club_get_experience_logs: <ListChecks className="w-4 h-4" />,
  club_list_members: <Users className="w-4 h-4" />,
  club_send_teams_notification: <Bell className="w-4 h-4" />,
  club_search_knowledge: <Search className="w-4 h-4" />,
};
...
const TOOL_LABELS: Record<string, string> = {
  ...
  club_save_preference: '好み登録',
  club_get_preference: '好み参照',
  club_save_experience: '体験ログ記録',
  club_get_experience_logs: '体験ログ参照',
  club_list_members: 'メンバー一覧',
  club_send_teams_notification: 'Teams通知',
  club_search_knowledge: 'ナレッジ検索',
};
...
export default function ChatArea() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // クラブメンバー情報をシステムプロンプトに含める
      const membersList = clubStore.members.map(m => `${m.id}: ${m.displayName}`).join(', ');

      // ナレッジ数を取得してプロンプトに含める
      const knowledgeCount = clubStore.knowledgeEntries?.length || 0;

      const normalSystemPrompt = `あなたは「ワイン・ビール部活」のAIアシスタントです。
メンバーのワインやビールの好みや体験を管理し、パーソナライズされた提案を行います。

## 登録済みメンバー
${membersList}

※ ユーザーが「私」「自分」と言った場合は memberId = "user-a" を使用してください。
※ 「Seiya」「せいや」と言った場合は memberId = "seiya" を使用してください。
※ 「草野」「kusano」と言った場合は memberId = "kusano" を使用してください。

## 主な機能

### 1. 有識者ナレッジ検索（最重要）
- **管理者ナレッジが${knowledgeCount}件登録されています**
- ユーザーの質問に店舗・ワイン・ビール・レビューが関連する場合は、**必ず最初に club_search_knowledge** を呼び出してください
- 検索結果に有識者（せーや・草野）のコメントが含まれる場合は、**必ず引用スタイルで表示**してください

#### 引用表示ルール
有識者コメントは以下の形式で必ず表示してください：

\`\`\`
> 「ここの店舗の最大のストロングポイントは徹底したグラス洗浄とマイスターによる注ぎ技術が圧倒的！」
> — せーや
\`\`\`

つまり Markdown の引用記法（> ）を使い、コメント本文を「」で括り、末尾に「— 登録者名」を付けてください。
複数の有識者コメントがある場合はすべて引用表示してください。

### 2. 好み登録・参照
- ユーザーが「〇〇が好き」「〇〇は苦手」と言ったら、club_save_preference で自動的に構造化して保存してください。
- 好みを聞かれたら club_get_preference で取得して回答してください。

### 3. 体験ログ
- 「今日〇〇ワインを飲んだ」「昨日Seiyaと〇〇ビールを飲んだ」等の発言から、日付・銘柄・種類・感想・店・同行者を抽出して club_save_experience で保存してください。
- 「今日」は実際の日付に変換してください（get_current_timeで現在日付を取得）。

### 4. Teams通知
- club_send_teams_notification でワイン・ビール関連のおすすめ情報をTeamsに通知できます。

### 5. その他のツール
- web_search: 最新のワイン・ビール情報、イベント、ニュースを検索
- get_current_time: 現在の日時を取得
- calculator: 計算

## 重要なルール
- **ワイン・ビール・店舗に関する質問を受けた場合は、必ず最初に club_search_knowledge を実行**し、管理者ナレッジを確認してから回答してください。
- ナレッジの有識者コメントは常に引用スタイル（> ）で表示してください。引用なしで要約したり省略したりしないでください。
- 好みや体験の登録時は、ユーザーの自然言語から構造化データを生成してツールで保存してください。
- 保存後は「記録しました」と内容を確認する返答をしてください。
- 日本語で回答してください。
- メンバーの好みに基づいた提案を積極的に行ってください。`;
...
日本語で回答してください。`;
      ...
      while (iterations < maxIterations) {
        ...
        for (const toolCall of response.toolCalls) {
          ...
          // Pass search config and tool context to executeTool
          const searchConfig = {
          ...
          const toolContext = {
            outlookEnabled: apiConfig.enableOutlook === true,
            teamsEnabled: apiConfig.enableTeams === true,
            clubStore: {
              members: clubStore.members,
              getPreferences: clubStore.getPreferences,
              setPreference: clubStore.setPreference,
              addExperienceLog: clubStore.addExperienceLog,
              getExperienceLogs: clubStore.getExperienceLogs,
              getMemberSummary: clubStore.getMemberSummary,
              getAllMembersSummary: clubStore.getAllMembersSummary,
              teamsWebhook: clubStore.teamsWebhook,
              searchKnowledge: clubStore.searchKnowledge as unknown as (query: string) => Array<Record<string, unknown>>,
              getKnowledgeForAI: clubStore.getKnowledgeForAI,
            },
          };
    ...
    <div className="flex-1 flex flex-col bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800 h-screen">
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b border-slate-200/50 dark:border-slate-700/50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-800 dark:text-white">Wine & Beer Club</h1>
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          {!currentChat || currentChat.messages.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center h-full py-12">
              <div className="w-20 h-20 bg-gradient-to-br from-amber-500 via-red-500 to-purple-600 rounded-2xl flex items-center justify-center mb-6 shadow-xl animate-pulse">
                <Wine className="w-10 h-10 text-white" />
              </div>
              <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">
                Wine & Beer Club Agent
              </h2>
              <p className="text-slate-500 dark:text-slate-400 text-center max-w-md mb-4">
                好みの登録、体験ログ、パーソナライズ提案までお任せください
              </p>
              
              {/* Available Tools */}
              <div className="flex flex-wrap justify-center gap-2 mb-8">
                {[...AVAILABLE_TOOLS, ...CLUB_TOOLS].map((tool) => (
                  <div
                    key={tool.name}
